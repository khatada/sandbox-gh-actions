...
async function checkFileMark(mark: string) {
  ...
  const customIgnorePattern = customIgnorePatternText
    .split(',')
    .map((token) => token.trim())
    .filter((token) => token.length > 0);

  const ignoreDirectories = ['build', 'dist', 'public', '.github', '.vscode'];
  const ignorePatterns = ['.min.js', '.d.ts', '.eslintrc.js', '.prettierrc.js'];
